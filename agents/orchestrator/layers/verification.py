"""Verification Layer — Quality scoring, citation validation, fact checking, LLM-as-Judge."""
...
def _rate_limit():
    """Enforce minimum interval between verification LLM calls."""
...
def _call_verify_llm(messages: list[dict], max_tokens: int = 512) -> tuple[str, int]:
    """Call verification LLM with retry and delay."""
    for attempt in range(3):
        if attempt > 0:
            _time.sleep(10 + 5 * attempt)
            logger.info("Verification LLM retry attempt %d/3 after delay", attempt + 1)
        _rate_limit()
        try:
            client = _get_llm()
            ...
            logger.warning("Verification LLM attempt %d/3 failed: %s", attempt + 1, e)
    return "", 0


def _extract_json_from_text(text: str) -> Any:
    """Extract JSON from model output that may contain thinking."""
...
def quality_score(draft: str, query: str) -> dict:
    """Score the draft on completeness, accuracy, clarity, and structure (1-10)."""
    content, tokens = _call_verify_llm(
        messages=[
            {
                "role": "system",
                "content": (
    ...
    if content:
        scores = _extract_json_from_text(content)
...
def validate_citations(draft: str, context_sources: list[dict]) -> dict:
    """Validate that citations in the draft reference real sources."""
    citation_pattern = r'\[Source (\d+)\]'
    found_citations = set(int(m) for m in re.findall(citation_pattern, draft))

    available_sources = set(range(1, len(context_sources) + 1))
    ...
    cited_ratio = len(found_citations) / max(total_claims // 3, 1)  # Expect ~1 citation per 3 sentences

    return {
        "valid_citations": list(valid),
        "invalid_citations": list(invalid),
        "uncited_sources": list(missing_coverage),
...
def llm_as_judge(draft: str, query: str, iteration: int) -> dict:
    """Overall pass/fail judgment with structured rubric."""
    content, tokens = _call_verify_llm(
        messages=[
            {
                "role": "system",
...
def run_verification(draft: str, query: str, context: list[dict], iteration: int) -> dict:
    ...
    total_tokens = (
        quality.get("tokens_used", 0)
        + facts.get("tokens_used", 0)
        + judge.get("tokens_used", 0)
    )

    # Aggregate failure reasons for improvement hints
    improvements = []
    ...
    if not facts.get("passed"):
        ...
        if hallucinations:
            improvements.append(f"Remove unsupported claims: {'; '.join(hallucinations[:3])}")
